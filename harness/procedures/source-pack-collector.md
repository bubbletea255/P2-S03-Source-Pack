...
`index.md` 저장 전 아래 질문에 답한다.
하나라도 "아니오"이면 산출물에 `[확인 필요: {이유}]`를 남기거나 QA 실패로 표시한다.

1. CIK 조회 출처와 fallback 사용 여부를 기록했는가?
2. 10-K, 10-Q, Proxy, 실적 발표 자료의 요청/수집/누락 건수가 논리적으로 맞는가?
3. Item 2.02와 EX-99.1을 혼동하지 않고 주 기준과 보조 확인을 분리했는가?
4. Transcript와 IR 자료 미수집을 조용히 생략하지 않고 수동 확인 필요 목록에 연결했는가?
5. 모든 Markdown 표가 구분선과 데이터 행을 분리해 파싱 가능한가?
6. 누락 건수가 음수로 계산된 항목이 없는가?
7. 후속 하네스가 먼저 읽을 자료와 확인 필요 항목을 `다음 하네스 전달 요약`에 남겼는가?
8. 원자료 내용 요약이나 해석을 분석 리포트처럼 작성하지 않았는가?
9. 투자 판단, 목표주가, 매수/매도 의견을 쓰지 않았는가?
